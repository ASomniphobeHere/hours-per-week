...
- [x] **7.3 School activity** (§3.3, §8.3) — inserted at S4 only, at `order: 0`, above sleep at the top of the stack. Weekly minimum 20 h across 5 workdays = 4 h/workday; adjustable upward only in 5 h weekly increments (25 → 5 h/day, 30 → 6 h/day) to a maximum of 40 h = 8 h/workday; decrement disabled at 20, increment disabled at 40; zero on weekend days, so the weekend stack is unaffected by the reveal. `locked` is true. The pack constraint gains `maxWeekly: 40` beside the existing `minWeekly` and `stepWeekly`.
  *AC: 38, 39*
...
- [x] **7.6 Overflow silence** (§7.6) — no over-by text, no toast, no error message, and no count of the excess, anywhere in the client. The toggle segment's bold red occupied-hours figure (step 4.2) is the sole numeric signal, and it never states the excess. The outcome text is not an exception: it describes the programme at the chosen pace and never characterises the participant's week, at any level and whether or not the stack fits.
  *AC: 42*

- [x] **7.7 Rebalance and completion** (§8.4, §3.4) — stripes vanish and confirm enables when `fits()` is true on **both** day types. No auto-advance on `fits()`: a participant who lands under 24 by accident gets to look at what they did, and may keep adjusting before confirming. Confirm POSTs `/complete` and enters S5. Emit `fits` at the transition. Lowering the pace in the sheet is a legitimate route to fitting, down to the 20 h floor — giving up outcome rather than hours is one of the decisions the exercise exists to surface, and it appears in cut order like any other `hours.change`.
  *AC: 43*

- [x] **7.8 The two edge outcomes** (§8.3, §8.4, §11) — a weekend already over 24 at S1 arrives at S4 with `fits()` false for a cause the forced workday view does not show; the red weekend segment plus the live toggle are what carry it, and both are required or confirm sits disabled with nothing on screen naming why. Separately, a participant with enough workday slack has Unallocated absorb school entirely: no stripes, confirm enabled on entry, cut order empty. That is a valid outcome and a real finding — do not manufacture a breach. **The threshold is now the pace they chose**, not a fixed 4 h: 20 h needs 4 h of slack and 40 h needs 8, so the same participant is slack-rich or breaching depending on a choice made one screen earlier.
  *AC: 44, 45*

**Stage 7 done when:** a breaching participant can rebalance to fit and confirm; a weekend-breaching participant can find and fix the cause; a slack-rich participant completes without ever seeing a stripe; and a participant can raise their pace to 40 h, see what it buys and what it costs per workday, and meet the stripes that follow. — **All four hold.** `components/participant/S4.test.tsx` drives the whole of S4 through the stage machine against a two-activity fixture, where a breach is seeded rather than derived and every edge is reachable in one line; `e2e/s4-reveal.spec.ts` drives the same route against the v1 pack and the real endpoints, because the one thing a fixture cannot prove is that the shipped content's own defaults land the workday just inside 24 h — which is what makes 4 h of school a squeeze and 8 h a considerably worse one. The whole exercise rests on that arithmetic being true of the pack rather than of a test.

**Six things decided in the building.**

**Which of S4's three screens is up is read off the schedule, not stored.** The commitment, the pace and the stack are steps inside one stage: §2.2's machine has five members and §6.2.2 counts sessions in them, so a sixth would be a stage the server has no column for. That leaves the question of what a refresh mid-S4 comes back to, and the answer was already in the record — school is `locked`, carries no questions and has no estimator, so the only way it can hold a value the participant authored is the pace screen's commit. `authored['school']` is therefore exactly the marker, and no second persisted field was added. A refresh before the commit returns to the commitment, which is right: it is a thing to have read rather than work to have done, on the same reasoning that restarts the S3 floor (§11).

**The day-type force runs at the transition, not at the stage.** §7.1 forces `wd` on entry to S4 and AC 15 promises the selection survives a refresh, and the two only agree if "entry" means the crossing rather than the condition. Forcing it wherever `stage === 's4'` would pin the toggle to the workday and take §8.3's weekend cause off the table; forcing it on a resume would cost AC 15 outright. So it sits in the one place S4 is entered from, beside `advance('s4')` — the same shape force-advance took in Stage 6, and for the same reason.

**The commit's `hours.change` is measured against the floor.** School enters the stack from nothing, so an event measured against what is on screen makes every commit a rise — including the default one nobody chose. `setWeekly` therefore takes the baseline as an argument: the pace screen passes `minWeekly` and logs nothing at 20 h, and the sheet passes the level it is leaving and logs every step. What §10 gets is one event that means *the participant chose a pace above the floor*, which is the half of step 7.2's pair the complete snapshot cannot recover. No `mode.direct` is emitted anywhere near it: that event says an activity was taken off its estimator, and school has never been on one.

**The rim is unconditional and the stripes are one element.** `Overflow` takes the scale and the day's total and nothing else, so there is no state a rule could key off and AC 41's "identical before and after breach" is a property of the code rather than of a test — the e2e still compares the computed rule across a rebalance, because a stylesheet can always grow a selector the component did not ask for. The stripes are a single clipped field from the rim to the bottom of the container: per-band stripes would restart the 6 px period at every boundary and would cover a straddling band in full. Both sit under the ruler, so §7.3's tick contrast is not re-opened by a red field over the numbers.

**`/complete` is delivered, not fired.** Stage 6 decided this for `/ready` and the argument is stronger one stage later: confirm is the end of the participant's work, S5 is not a screen they can retry from, and the complete snapshot is what every per-activity delta in the debrief is taken against (§10). So `deliver.ts` grew a shared retry and two thin callers rather than a second copy of the backoff, and the transition stays immediate — a spinner after the last press would be the network wait §6.3 keeps out of the room, one stage on.

**S5 draws nothing, and the middle three rungs stay placeholders** (both decided with the user, 2026-08-26). §9's table names no `s5` key and the spec describes no screen, so anything on it would be a string the client invented rather than content the pack owns; it is a full-height empty page, and §10.4's debrief is what eventually lands there. The ladder ships as the pack has it — the endpoints are the user's words and 25, 30 and 35 read "Placeholder copy for N hours a week", which §8.3 explicitly admits as a complete and testable build. Authoring them is a pack edit and no client change, which is the property the outcome keys were addressed by weekly value to get.

**Two things the stage did not need.** No new copy key was added: the eight the two-screen amendment named were written into the pack under PR #9, and the control reuses `unit.hours`, `a11y.decrease` and `a11y.increase` from §9's own additions. And `weeklyLevels` moved from `lib/pack/validate.ts` to `lib/domain/constraints.ts`, where the rest of the ladder already lived — the stepper walks the same levels the required-copy check is derived from, and a component reaching into the validator to find out how many rungs there are would be the second definition that rule exists to prevent.
...
- [x] **8.7 The landing screen** (§6.2, §6.2.1 as amended) — `/facilitate`, with no room: a **New room** control that `POST`s `/room`, remembers the result in this browser, and lands on the new room's console. Beneath it, the rooms this browser has already created — join code and creation time, each a link back to its console, each with a quiet forget control. Absent entirely when there is nothing to list. One press, not two: `StageButton` arms because the flip force-advances a room and cannot be undone, whereas a room nobody joins is inert. Disabled while the request is in flight, because two rooms means a facilitator reading one code aloud while the console shows the other.
  *AC: 49a*

**Stage 8 done when:** the console drives a real room end to end from a laptop and from a 375 px phone, survives a reload and a dropped poll, writes exactly one `stage.open` row, and can be opened without a terminal. — **All five hold.** `e2e/console.spec.ts` runs the console against a real room with a real phone joining in a second tab, on both the desktop and the mobile project, because the one thing a stub cannot prove is that the three numbers on screen are the room; `components/facilitator/Console.test.tsx` drives the screen and both presses through an injected `fetch`, where a dropped poll and a failed POST are one line each; and the `stage.open` row is asserted in `app/api/routes.test.ts`, which is the only place a facilitator's double-press and a latecomer arriving after the flip can be put in the same test. Step 8.7's landing screen closes the last of it: the e2e creates a room from the button, joins a phone on the code the console shows, and comes back to that room from the list — the one sequence neither a stub nor a route test can prove end to end.
...
**Two more, from the addendum** (decided with the user, 2026-08-27, after the console was found to be reachable only by hand).

**The remembered list is the browser's, not the server's.** The obvious build is `GET /rooms` and a table; the obvious build is wrong. §6.2.6 accepts one boolean, in one room, for one supervised workshop, and that argument rests entirely on `roomId` being neither guessable nor listable (**RD-2**) — an endpoint returning every room hands the flag for all of them to anyone who reaches the unauthenticated console. `localStorage` gets the facilitator back to their own rooms and costs the threat model nothing, at the price of a shortcut that a cleared browser loses. `consoleUrl` is still the durable handle, and §6.2.1 now says so in writing so the endpoint is not re-proposed.

**The landing screen navigates with the document, not the router.** `useRouter` and `next/link` would have been the reflex, and both throw outside an app-router context — which would have cost every test on this screen a router harness to prove a soft transition on a page reached once a workshop. The console is `force-dynamic` and starts its poll on mount, so a full navigation loses nothing; `navigate` is injected with `location.assign` as its default, the way `fetchImpl` is injected everywhere else in `lib/console`. The list is read through `useSyncExternalStore` rather than an effect for the neighbouring reason: the server has no browser to ask, so an empty list on the server and the real one after hydration is the shape the hook exists for, and the component ends up holding no state at all.
...
- [x] **10.6 The S5 summary — what it cost** (§10, §2.2, §9 as extended) — S5 is the first screen in this build with nothing behind it: §2.2's stage table gives it `— | —`, §12 names no criterion for it, and §9's copy table has no `s5.*` key. It gets one screen, and that screen is §10's first derived field turned around and shown to the person who produced it. The debrief reads per-activity delta to learn what the room gave up; the participant has at least as much claim on it, and has just spent five minutes deciding it.
  ...
  **Rows are cuts only** — one per activity whose hours fell, `from → to`, in the utility face with `tabular-nums`. School is not a row: it went up, and it is the reason for the list rather than a member of it, so it is named once in `s5.title` and appears nowhere else on the screen. Nor are increases: a participant who nudged sleep up while cutting elsewhere is not being asked what their week gained. The screen answers one question, and every row on it is part of that answer.

  **Day types.** A row is per activity *per day type*, because a cut on a workday and a cut on a weekend day are two different decisions and summing them would report neither. The common case is entirely workday — S4 forces `wd` (step 7.1) and school is workday-only — so the day label is carried on the rows **only when the list contains a weekend row at all**; a repeated "workday" down a list that is nothing else is chrome, not information. Labels reuse `toggle.wd` / `toggle.we` rather than minting a private pair, on the Stage 5 reasoning: §9's rule is that no string is hardcoded, not that every surface owns its own copy of one.

  **The no-cut case gets its own screen, not an empty list.** A participant whose workday slack covered the pace they chose cut nothing — Unallocated absorbed school, silently and without an event (§7.8, step 7.8). Their screen states the school figure and says it came out of time they had not allocated, and that nothing else in their week moved. That is the one place the school number appears as a figure on this screen, and it appears exactly where there is otherwise nothing to show. It is a true statement about their answers and a real finding (§8.4, §11) — not a consolation, and it does not characterise the week that produced it.

  **What the screen knowingly does not say.** A participant with 2 h of slack and a 4 h pace cut 2 h and reads a short list, and that list understates what school cost them by exactly the slack it ate first. §10 says the debrief must read cut order against slack at finish for this reason. The participant's screen does not: annotating their list with what they gave up without deciding it is the client telling them something about their week, which is what §7.6's silence rule exists to prevent. The debrief gets the annotation; the participant gets their decisions.

  **Rendering.** A list, not the instrument — no bands, no hues, no spines, no stack. Partly because there is nothing left to edit at S5 and re-rendering the stack invites the attempt, and partly because the stack is a picture of a week and this is a record of a change to one. No controls: S5 is terminal, and its only affordance is the options tab (10.7).
...
- [x] **10.7 The options tab at S5** (§7.9, §5) — the tab, and Start over inside it, are present on the summary screen. §7.9 contradicts itself here — "**Editor only.** The tab appears at S2 and after" — and step 4.8 shipped the first sentence. Resolved in favour of the second: at S5 the run is over and there is no other way off the screen, so a phone that finished is a phone that is finished for good. That is wrong in a room where someone wants to run it again, and it is wrong in rehearsal (Stage 12), where a handful of phones are meant to drive the same room repeatedly.
  ...
  **The reset itself does not change** — server asked first, the row and its snapshots and events deleted outright, a replacement session minted in the same room (step 4.8). One thing does: §7.9 requires the confirmation to name what is lost, and at S5 what is lost is not what is lost mid-run. A second body key, `options.reset.body.complete`, states that the finished result goes with the answers.

  **The cost, stated rather than designed around.** A reset at S5 removes a completed run from the room: `ready` and `total` both fall by one on the console (step 8.2), and the debrief loses that participant entirely. Accepted knowingly — deletion is what keeps `total` counting one row per participant (§6.2.2), and the alternative, preserving the finished row beside a fresh one, puts two rows and a phantom stage count against one person. The facilitator sees a ready count go down, which the console reports without comment, and 10.5's table is derived from the rows that exist.
  *AC: 22a (as amended below), 22b and 22c re-verified from S5 — a reset after `/complete` leaves one row in the room and destroys nothing on a network failure.*
...
**Cuts only, and from → to.** Two readings were open. The first, whether school and any increases join the list: they do not — the screen answers what the commitment cost, and an activity that grew is not part of that answer, so school is named in the heading as the reason and nothing else moves onto the list. The second, whether §7.6's silence reaches the figures: it does not. §7.6 forbids the client stating the *excess* — an over-by count, a toast, a characterisation of the week. A participant's own hours are not that; they were on the bands and on the toggle for the whole rebalance, and withholding them at the end would withhold the one measurement the exercise exists to produce from the person who produced it.

**Two spec sections are amended, and the build follows this plan rather than them.**

§7.9's **"Editor only. The tab appears at S2 and after"** cannot be both. Step 10.7 takes the second sentence: S2, the S4 stack, S5. **AC 22a's test changes with it** — from "present in the editor and absent everywhere else" to *present at S2, at the S4 stack and at S5, and absent on the join screen, the intro, S1, S3 and the two reveal screens*. The property 22a protects is that the tab is unreachable where a destructive control has no business being, and the amended list states that property exactly rather than through a word — "the editor" — that S5 made ambiguous. 22a's second half, that the tab takes no part in the `pxPerHour` measurement, is untouched: it is `position: fixed` at S5 as everywhere else.

§2.2's **S5 row and §12's silence** are what leave S5 undefined. Step 10.6 defines it, and §9's table gains four keys plus `options.reset.body.complete`, on the same reasoning as the fourteen Stage 3 added, the four Stage 4 added and the eight Stage 7 added: they are participant-facing strings the spec's table does not name, and a replacement pack that omits them leaves a room reading raw key names.

**The trade taken knowingly.** Start over at S5 can delete a completed run, and with it a row of the debrief this stage exists to produce. It is the same reset §7.9 already specifies and the same deletion §6.2.2 already requires; the alternatives were a tab that appears but does nothing, or two session rows for one person. Neither is better than a facilitator watching a ready count fall by one. See Risks.
...
| A participant starts over from S5 and deletes a finished run | 10.7 | The cost of one reset path and of §6.2.2's one-row-per-participant rule. The confirmation names the finished result before it goes, and it sits two taps and a sentence behind a sliver. The symptom is a ready count that falls, which the console reports without comment; the debrief is derived from the rows that exist. |
